...
export const Program = ({ program, ...rest }: ProgramItem) => {
	...
	return (
		<ProgramBox width={styles.width} style={styles.position} className="planby-program group transition-all">
			<ProgramContent width={styles.width} isLive={isLive} className="planby-program-content transition-all">
				<div className="flex justify-between items-center transition-all">
					...
					<ProgramStack className="planby-program-stack transition-all">
						<div className="inline-flex gap-2 items-center mb-1">
							<p className="bg-emerald-500 rounded-full w-fit p-1 text-white font-heading">{pointValue}</p>
							<p className="font-heading text-sm">{title}</p>
						</div>
            <p className="group-hover:block  hidden">{details}</p>
						<ProgramText className="planby-program-text group-hover:hidden transition-all">{location}</ProgramText>
